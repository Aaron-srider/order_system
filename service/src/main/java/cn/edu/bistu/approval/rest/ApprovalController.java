...
public class ApprovalController extends BaseController {
    ...
    public Result approveWorkOrder(
            @RequestBody @Validated(Update.class) ApprovalRecord approvalRecord,
            HttpServletRequest req) {

        setUpApprovalRecord(req, approvalRecord);

        if (approvalRecord.getOperation().equals(ApprovalRecord.PASS)) {
     ...
     * 获取待审批的所有工单
     * @param pageVo 分页信息
     * @param workOrderVo 工单信息，包含工单标题（title）
     * @return 返回工单的分页数据
     */
